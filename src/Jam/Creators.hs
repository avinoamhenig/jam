...
createUnit' :: Exp
createUnit' = UnitExp (TyDefType unitType []) empty

createNum :: Int -> State Prog Exp
createNum x = return $ createNum' x

createNum' :: Int -> Exp
createNum' x = NumExp (TyDefType numberType []) empty x
...
createLambda argName = do
  ...
  return $ LambdaExp (TyDefType functionType [t1, t2]) -- type
                     empty                             -- bindings
...
createApp = do
  ...
  return $ AppExp t2 empty -- type, bindings
                  (_btm $ TyDefType functionType [t1, t2]) -- func
                  (_btm t1) -- arg
...
createIf = do
  ...
  return $ IfExp t empty -- type, bindings
                 (_btm $ TyDefType boolType []) -- condition
                 (_btm t) -- then
                 (_btm t) -- else
